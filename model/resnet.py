...
class ResNet_Seg(nn.Module):
    '''
    DM: ResNet with no downsampling. Final representation is for each pixel, use for semantic segmentation.
    '''
    def __init__(self, block, layers, num_classes=1000):
        self.inplanes = 64
...
class ResNet_FCN_coarse(nn.Module):
    '''
    Normal ResNet34 + a x32 upsample layer in the end
    '''
    def __init__(self, block, layers, num_classes=1000):
        self.inplanes = 64
        super(ResNet, self).__init__()
        ...
        self.upscale = nn.Conv2d(num_classes)
        #self.avgpool = nn.AvgPool2d(7)
        #self.fc = nn.Linear(512 * block.expansion, num_classes)
...
def resnet34_seg(pretrained=True):
    """Constructs a ResNet-34 model.
    Args:
      pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_Seg(BasicBlock, [3, 4, 6, 3])
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet34']))
    return model
